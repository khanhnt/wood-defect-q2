#!/usr/bin/env python3
"""Preprocess the main large-scale dataset into a compact server-upload format."""
...
from src.datasets.large_scale_dataset import preprocess_large_scale_for_server
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/preprocess_main.yaml",
...
def main() -> None:
    ...
    logger.info("Preprocessing main dataset from %s", config.get("root_dir"))
    logger.info("Processed output root: %s", config.get("processed_root_dir"))
    result = preprocess_large_scale_for_server(config)

    logger.info("Source images: %d", result["summary"]["num_source_images"])
    logger.info("Processed tiles: %d", result["summary"]["num_processed_images"])
