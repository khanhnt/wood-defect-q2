...
\Author{
  Khanh Nguyen-Trong\textsuperscript{1,2}
}
...
\abstract{This study evaluates wood surface defect detection under three settings: a curated public-benchmark protocol, a broader in-domain seven-class protocol, and transfer to a low-resource target domain. We compare lightweight Faster R-CNN + MobileNetV3-FPN detectors, a compact YOLOv8s baseline, and two small-object-oriented YOLO variants. Under matched 50-epoch runs, YOLOv8s outperforms the MobileNet-based two-stage baselines, whose best AP50 values are 74.52\% on the curated benchmark and 75.15\% on the broader seven-class protocol. On the curated benchmark, \texttt{Y0-3600e200} reaches 84.38\% AP50, while the matched 200-epoch reruns \texttt{Y1-e200} and \texttt{Y2-e200} reach 83.70\% and 83.93\%, respectively. The broader seven-class protocol remains harder: \texttt{Y0-full} reaches 80.39\% AP50 at 50 epochs and \texttt{Y0-full\_e200} reaches 81.16\% in native YOLO validation. These results place \texttt{Y0-3600e200} near the upper end of the closest public results reported on related benchmark variants.

On VNWoodKnot, the seed-42 transferred run outperforms target-only training on the held-out test split and converges faster on validation. Repeated-seed evaluation over four seeds, however, does not confirm a held-out-test AP50 gain for source initialization under the same 50-epoch budget. Across all three settings, benchmark curation, protocol breadth, and dataset shift alter the observed ranking almost as much as the local YOLO modifications examined here.}
...
Table~\ref{tab:main-matrix} compares the broader seven-class reference, the curated-benchmark baseline, and the matched 200-epoch reruns. Figures~\ref{fig:y0-classwise-ap50}--\ref{fig:yolo-progress} show the class-wise AP50 profile and training dynamics of the final curated-benchmark model.

The strongest result in Table~\ref{tab:main-matrix} is \texttt{Y0-3600e200}, which reaches 0.8438 AP50, 0.8088 precision, and 0.7911 recall on the curated benchmark. The matched broader seven-class rerun, \texttt{Y0-full\_e200}, reaches 0.8116 AP50, so the broader protocol remains clearly harder within the same detector family. The 50-epoch single-run references show the same protocol gap: \texttt{Y0-full} reaches 0.8039 AP50, whereas \texttt{Y0-3600} reaches 0.8357 AP50.

The matched 200-epoch ablations do not change that ranking. \texttt{Y1-e200} reaches 0.8370 AP50 and \texttt{Y2-e200} reaches 0.8393 AP50, so neither variant surpasses \texttt{Y0-3600e200}. Among these two ablations, the WNIoU-style branch comes closest to the final curated-benchmark baseline.

Among the recent works built on the same benchmark family, ODCA-YOLO and SiM-YOLO report 78.5\% and 78.4\%, BPN-YOLO reports 81.8\%, and FDD-YOLO reports 82.3\% \citep{odca_yolo,bpn_yolo,sim_yolo,fdd_yolo}. Hoang \textit{et al.} reported 81.2\% AP50 on a relabeled benchmark derived from the Kodytek Zenodo release using an area-attention detector \citep{cmc_wood_area_attention}. Our final YOLOv8s model reaches 84.38\% AP50 on the curated benchmark, placing \texttt{Y0-3600e200} near the upper end of the closest public results reported on related benchmark variants.
...
Table~\ref{tab:external} summarizes a single-seed target-domain adaptation comparison on VNWoodKnot. The held-out target-domain test split used here contains 229 images, 155 target boxes, and 75 negative \texttt{knot\_free} images. Under the seed-42 run, \texttt{T1} reaches 0.8312 AP50, whereas \texttt{T0} reaches 0.7753. Precision also rises from 0.5370 to 0.6730, recall rises from 0.8903 to 0.9161, and the number of predictions drops from 257 to 211. The validation trajectories show the same pattern: \texttt{T1} reaches 0.70 AP50 by epoch 9 and 0.80 by epoch 31, whereas \texttt{T0} reaches the same thresholds at epochs 19 and 40.
...
Repeated-seed analyses over seeds 42, 43, 44, and 45 show that the short-budget ranking remains stable in-domain. In native YOLO validation, \texttt{Y0} reaches $0.8379 \pm 0.0050$ AP50, \texttt{Y2} reaches $0.8355 \pm 0.0033$, and \texttt{Y1} reaches $0.8299 \pm 0.0082$. Under the shared repo evaluator on the held-out curated test split, the ordering becomes \texttt{Y0} ($0.7791 \pm 0.0052$) $>$ \texttt{Y1} ($0.7757 \pm 0.0084$) $>$ \texttt{Y2} ($0.7725 \pm 0.0042$). None of the local YOLO refinements therefore yields a robust 50-epoch held-out-test AP50 gain over the compact baseline.

The target-domain picture differs. On VNWoodKnot validation, \texttt{T1} reaches $0.8535 \pm 0.0101$ AP50 and \texttt{T0} reaches $0.8444 \pm 0.0088$, so source initialization remains slightly better on average under the same 50-epoch budget. On the held-out test split, however, the ordering reverses: \texttt{T0} reaches $0.7909 \pm 0.0136$ AP50, whereas \texttt{T1} reaches $0.7730 \pm 0.0409$. Under this budget, transfer changes optimization behavior and the precision--recall trade-off, but it does not yield a robust held-out-test AP50 gain across seeds.

\begin{table*}[t]
  \centering
  \caption{Validation-side repeated-seed results for target-domain adaptation under the 50-epoch setting. Reported values are mean $\pm$ standard deviation over seeds 42, 43, 44, and 45.}
...
The strongest result in the evaluated design space comes from the compact YOLOv8s baseline. On the curated benchmark, the long-schedule run reaches 84.38\% AP50, which places the model near the upper end of the closest public results reported on related benchmark variants. We do not frame this as a strict SOTA claim because the reconstructed benchmark, checkpoint policy, and evaluation details are not identical across studies.
...
\section[\appendixname~\thesection]{Supplementary Robustness Tables} \label{sec:appendix-robustness}
%\appendix
%\section{Supplementary Robustness Tables}
%\label{sec:appendix-robustness}
