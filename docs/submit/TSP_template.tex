...
\usepackage{cmap}   % đặt rất sớm, gần đầu preamble
...
\Author{Khanh Nguyen-Trong\textsuperscript{1,2}}
...
\AuthorNames{Khanh Nguyen-Trong}
% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address{%
\textsuperscript{1} Intelligent Computing for Sustainable Development Lab, Posts and Telecommunications Institute of Technology (PTIT), Hanoi, Vietnam \\
\textsuperscript{2}Faculty of Information Technology, Posts and Telecommunications Institute of Technology (PTIT), Hanoi, Vietnam
}%
% Contact information of the corresponding author
\corres{Corresponding Author:  Khanh Nguyen-Trong, Email: khanhnt@ptit.edu.vn}
...
\abstract{Automated wood surface defect detection is difficult to deploy reliably because defects are often small, low-contrast, and visually confounded by natural wood texture, while evaluation results can vary substantially with benchmark design and domain shift. To address this issue, we conduct a comparative study across three practically relevant conditions: a curated seven-class benchmark, a broader in-domain seven-class protocol derived from the same source dataset, and supervised adaptation to a low-resource Vietnamese target domain. We compare lightweight two-stage detectors based on Faster R-CNN with MobileNetV3-FPN against a compact YOLOv8s baseline and two small-object-oriented YOLO refinements that enhance feature resolution or localization supervision. The results show that detector family choice matters more than the local YOLO modifications examined here. Across in-domain experiments, the compact YOLOv8s baseline delivers the strongest performance, achieving 84.38\% AP50 on the curated benchmark, whereas performance drops to 81.16\% AP50 under the broader protocol, indicating that benchmark curation materially influences the apparent difficulty of the task and the relative strength of competing models. In the target-domain setting, source-initialized fine-tuning improves optimization behavior and can outperform target-only training in a representative single run, but repeated-seed evaluation does not confirm a consistent held-out test advantage under the same adaptation budget. These findings suggest that conclusions drawn from a single curated benchmark may overstate model robustness, and that for wood defect detection, protocol breadth and source-to-target shift should be treated as central evaluation factors rather than secondary experimental details.}
% List 3 to 10  pertinent keywords specific to the article, yet reasonably common within the subject discipline.
\keyword{Wood surface defect detection; lightweight detector comparison; MobileNetV3-FPN; YOLOv8; benchmark sensitivity; industrial visual inspection; domain shift}  

\begin{document}
\section{Introduction}
\label{sec:introduction}
% Automatic surface inspection is a persistent requirement in industrial production because manual inspection is costly, subjective, and difficult to scale. In wood processing, the problem is even harder than in many manufactured materials because the substrate is inherently heterogeneous: grain, resin, knots, cracks, pith, stains, and growth patterns create large appearance variation even before true defects are considered. This challenge belongs to a broader wood-vision research landscape that spans species recognition, texture analysis, grading, and material characterization, where strong appearance variability, limited labeled data, and domain-specific acquisition settings have long been recognized as central obstacles \citep{de_geus_2021,herrera_poyatos_2024}. Kodytek \textit{et al.} note that surface defects do not merely lower visual quality; they can also degrade strength and stiffness, which directly affects material utilization and economic value in downstream processing \citep{kodytek_dataset}.

Automatic surface inspection is important in industrial production because manual inspection is costly, subjective, and difficult to scale. In wood processing, the problem is particularly challenging because natural grain, resin, knots, cracks, pith, and staining introduce substantial appearance variation even before true defects are considered. Surface defects are therefore relevant not only to visual grading but also to downstream material value and utilization \citep{kodytek_dataset}.

In practice, an inspection system must therefore detect visually weak defects, separate semantically similar subtypes, and remain stable across production batches while using detector choices that are realistic for deployment rather than only optimal on a curated benchmark.
...
From a vision perspective, wood surface defect detection is challenging for several reasons. Wood surface defect detection is challenging because many targets are small, low-contrast, elongated, or only partially visible. In addition, several categories such as live knots, dead knots, knots with cracks, and cracks are semantically close, the class distribution is long-tailed, and deployment data often differ from benchmark data in scale, framing, and label space.

% ?First, many targets are small relative to the board image and may appear low-contrast, elongated, or only partially visible, which makes localization unstable under repeated downsampling. Second, several categories such as live knots, dead knots, knots with cracks, and cracks are semantically close and visually entangled, so subtype confusion is easy even when a detector fires near the right region. Third, the class distribution is long-tailed, which makes rare but operationally meaningful defects hard to learn. Fourth, deployment data rarely match benchmark data in framing, scale, label taxonomy, or defect prevalence, so transfer behavior matters as much as in-domain validation.

Recent wood-defect studies mainly pursue detector-side improvements, typically by modifying feature fusion, attention, small-target handling, or localization loss \citep{odca_yolo,bpn_yolo,sim_yolo,fdd_yolo,an_cwb_yolov8}. However, it remains unclear whether the resulting detector rankings remain stable when the evaluation protocol is broadened or when models are transferred to a lower-resource target domain. Across these papers, the same trade-offs keep resurfacing: small targets disappear at coarse scales, localization is fragile on weak or elongated defects, and the detector still has to fit a practical compute budget.
...
\begin{figure*}[t]
\centering
\subfloat[Representative defect categories from the public benchmark domain.\label{fig:dataset-overview-public}]{
    \includegraphics[width=0.48\textwidth]{figures/wood_defect_types.png}
}\hfill
\subfloat[Representative images from VNWoodKnot.\label{fig:dataset-overview-vn}]{
    \includegraphics[width=0.48\textwidth]{figures/vnwoodknot_triptych.png}
}
\caption{Visual comparison of the two datasets used in this study.}
...
\begin{table*}[t]
  ...
  \begin{tabularx}{0.94\textwidth}{>{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
    \toprule
...
\begin{equation}
\mathrm{CIoU} = \mathrm{IoU}
- \frac{\rho^2(\mathbf{b}, \mathbf{b}^{*})}{c^2}
- \alpha_{\mathrm{ciou}} v
\end{equation}
...
\begin{itemize}
  \item On the broader full seven-class protocol, the high-resolution MobileNet baseline reaches 75.15\% AP50, while \texttt{Y0-full} reaches 80.39\% AP50.
  \item On the curated benchmark, the best high-resolution MobileNet variant (\texttt{E1}) reaches 74.52\% AP50. Under the same benchmark family, the final compact YOLO baseline reaches 84.38\% AP50 after 200 epochs.
  \item The low-capacity MobileNet baseline reaches 71.46\% AP50 on the broader seven-class validation setting and serves mainly as an exploratory reference showing that simply keeping a lightweight Faster R-CNN + MobileNet family does not close the gap to the stronger compact YOLO baseline.
\end{itemize}
...
\begin{table}[b]
  ...
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.15\linewidth} >{\raggedright\arraybackslash}p{0.1\linewidth} c X}
    \toprule
    \textbf{Family} / \textbf{Run} & \textbf{Protocol} & \textbf{AP50} & \textbf{Description} \\
    \midrule
    E1 & Full seven-class & 71.46\% & Exploratory low-capacity two-stage reference selected under the same in-domain protocol. \\
   B1 & Full seven-class & 75.15\% & Primary lightweight two-stage reference for the broader seven-class protocol. \\
    \texttt{Y0-full\_e200} (YOLOv8s) & Full seven-class & \best{81.16\%} & Main compact one-stage reference for the broader seven-class protocol after the matched 200-epoch rerun. \\
    \texttt{Y0-3600e200} (YOLOv8s) & Curated benchmark & \best{84.38\%} & Long-schedule compact YOLOv8s run used for literature-facing comparison. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Table~\ref{tab:main-matrix} compares the broader seven-class reference, the curated-benchmark baseline, and the matched 200-epoch reruns. Figures~\ref{fig:y0-classwise-ap50}--\ref{fig:yolo-progress} show the class-wise AP50 profile and training dynamics of the final curated-benchmark model. The strongest result in Table~\ref{tab:main-matrix} is \texttt{Y0-3600e200}, which reaches 84.38\% AP50, 80.88\% precision, and 79.11\% recall on the curated benchmark. The matched broader seven-class rerun, \texttt{Y0-full\_e200}, reaches 81.16\% AP50, so the broader protocol remains clearly harder within the same detector family. The 50-epoch single-run references show the same protocol gap: \texttt{Y0-full} reaches 80.39\% AP50, whereas \texttt{Y0-3600} reaches 83.57\% AP50.

The matched 200-epoch ablations do not change that ranking. \texttt{Y1-e200} reaches 83.7\% AP50 and \texttt{Y2-e200} reaches 83.93\% AP50, so neither variant surpasses \texttt{Y0-3600e200}. Among these two ablations, the WNIoU-style branch comes closest to the final curated-benchmark baseline.

Among recent studies evaluated on the same benchmark family, ODCA-YOLO and SiM-YOLO reported AP50 values of 78.5\% and 78.4\%, respectively, while BPN-YOLO and FDD-YOLO reported 81.8\% and 82.3\%, respectively \citep{odca_yolo,bpn_yolo,sim_yolo,fdd_yolo}. Hoang \textit{et al.} reported 81.2\% AP50 on a relabeled benchmark derived from the Kodytek Zenodo release using an area-attention detector \citep{cmc_wood_area_attention}. Our final YOLOv8s model achieved 84.38\% AP50 on the curated benchmark, placing it above the AP50 values reported in these recent studies on closely related benchmark variants.

\begin{table}[t]
  \centering
  \caption{Focused YOLO comparisons across the broader seven-class protocol, the curated benchmark, and the matched 200-epoch reruns. AP50 is the primary metric.}
  \label{tab:main-matrix}
  \small
  \begin{tabularx}{0.72\linewidth}{l l c c c c}
    \toprule
    \textbf{ID} & \textbf{Dataset} & \textbf{Epochs} & \textbf{AP50} & \textbf{Prec.50} & \textbf{Rec.50} \\
    \midrule
    \texttt{Y0-full} & full seven-class & 50 & 80.39\% & 74.89\% & 77.42\% \\
    \texttt{Y0-3600} & curated benchmark & 50 & \best{83.57\%} & 79.46\% & 79.95\% \\
    \texttt{Y0-full\_e200} & full seven-class & 200 & 81.16\% & 76.43\% & 75.85\% \\
    \texttt{Y1-e200} & curated benchmark & 200 & 83.7\% & 79.53\% & 78.27\% \\
    \texttt{Y2-e200} & curated benchmark & 200 & 83.93\% & 79.23\% & 79.16\% \\
    \texttt{Y0-3600e200} & curated benchmark & 200 & \best{84.38\%} & 80.88\% & 79.11\% \\
    \bottomrule
\end{tabularx}
\end{table}

Figure~\ref{fig:y0-classwise-ap50} shows the class-wise AP50 of \texttt{Y0-3600e200}. The strongest classes are marrow (89.1\%) and knot missing (88.2\%), followed by resin (78.2\%). The weakest class is live knot (67.8\%), while dead knot (73.6\%), knot with crack (75\%), and crack (75.4\%) occupy the middle range. This ranking is consistent with the semantic ambiguity discussed in Section~\ref{sec:introduction}: knot-related categories remain harder than the most visually distinctive defect types.
...
Table~\ref{tab:external} summarizes the target-domain adaptation comparison on VNWoodKnot. The held-out target-domain test split used here contains 229 images, 155 target boxes, and 75 negative \texttt{knot\_free} images. Under the seed-42 run, \texttt{T1} reaches 83.12\% AP50, whereas \texttt{T0} reaches 77.53\%. Precision also rises from 53.7\% to 67.3\%, recall rises from 89.03\% to 91.61\%, and the number of predictions drops from 257 to 211. The validation trajectories show the same pattern: \texttt{T1} reaches 70\% AP50 by epoch 9 and 80\% by epoch 31, whereas \texttt{T0} reaches the same thresholds at epochs 19 and 40.

This seed-42 comparison shows that the target task is learnable within a modest training budget and that source initialization can produce a stronger adapted run. The repeated-seed analysis below is used to determine whether that advantage persists across seeds.

\begin{table}[t]
  \centering
  \caption{Target-domain adaptation on VNWoodKnot under a fixed 50-epoch budget, with seed 42.}
  \label{tab:external}
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.05\linewidth} >{\raggedright\arraybackslash}p{0.2\linewidth} c c c X}
    \toprule
    \textbf{ID} & \textbf{Setting} & \textbf{AP50} & \textbf{Prec.50} & \textbf{Rec.50} & \textbf{Notes} \\
    \midrule
    \texttt{T0} & target-only supervised training & 77.53\% & 53.7\% & 89.03\% & held-out test (229 images, 155 targets), seed 42, 257 predictions \\
    \texttt{T1} & \texttt{Y0-3600e200} $\rightarrow$ fine-tune & \best{83.12\%} & \best{67.3\%} & \best{91.61\%} & held-out test (229 images, 155 targets), seed 42, 211 predictions \\
    \bottomrule
  \end{tabularx}
\end{table}

Figure~\ref{fig:vnwoodknot-transfer} visualizes the same seed-42 comparison. The largest class-wise AP50 gain appears on \texttt{live\_knot}, where AP50 rises from 71.06\% to 83.24\%. For \texttt{dead\_knot}, AP50 changes little, but precision and recall still improve. The validation curves show that \texttt{T1} reaches the same accuracy range earlier than \texttt{T0}.
...
Repeated-seed analyses over seeds 42, 43, 44, and 45 show that the short-budget ranking remains stable in-domain. In native YOLO validation, \texttt{Y0} reaches $83.79\% \pm 0.0050$ AP50, \texttt{Y2} reaches $83.55\% \pm 0.0033$, and \texttt{Y1} reaches $82.99\% \pm 0.0082$. Under the shared repo evaluator on the held-out curated test split, the ordering becomes \texttt{Y0} ($77.91\% \pm 0.0052$) $>$ \texttt{Y1} ($77.57\% \pm 0.0084$) $>$ \texttt{Y2} ($77.25\% \pm 0.0042$). None of the local YOLO refinements therefore yields a robust 50-epoch held-out-test AP50 gain over the compact baseline.

The target-domain picture differs. On VNWoodKnot validation, \texttt{T1} reaches $85.35\% \pm 0.0101$ AP50 and \texttt{T0} reaches $84.44\% \pm 0.0088$, so source initialization remains slightly better on average under the same 50-epoch budget. On the held-out test split, however, the ordering reverses: \texttt{T0} reaches $79.09\% \pm 0.0136$ AP50, whereas \texttt{T1} reaches $77.3\% \pm 0.0409$. Under this budget, transfer changes optimization behavior and the precision--recall trade-off, but it does not yield a robust held-out-test AP50 gain across seeds.

\begin{table*}[t]
  \centering
  \caption{Results for target-domain adaptation under the 50-epoch setting. Reported values are mean $\pm$ standard deviation over seeds 42, 43, 44, and 45.}
  \label{tab:multiseed-current}
  \scriptsize
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{0.85\textwidth}{>{\raggedright\arraybackslash}p{0.35\textwidth} 
  %>{\raggedright\arraybackslash}p{0.16\textwidth} 
  >{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}p{0.16\textwidth} X}
    \toprule
    Setting  & AP50 & Prec.50 & Rec.50 \\
    \midrule
    \texttt{T0} (target-only) & $84.4\% \pm 0.009$ & $76.2\% \pm 0.037$ & $82\% \pm 0.043$ \\
    \texttt{T1} (\texttt{Y0-3600e200} $\rightarrow$ fine-tune)  & $85.4\% \pm 0.010$ & $79.8\% \pm 0.024$ & $81.9\% \pm 0.011$ \\
    \bottomrule
  \end{tabularx}
\end{table*}
...
\begin{table}[t]
  \centering
  \caption{Held-out-test repeated-seed results for target-domain adaptation on VNWoodKnot. All rows correspond to 50-epoch runs, and reported values are mean $\pm$ standard deviation over seeds 42, 43, 44, and 45.}
  \label{tab:multiseed-transfer-test}
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{0.9\linewidth}{>{\raggedright\arraybackslash}p{0.30\linewidth} >{\raggedright\arraybackslash}p{0.16\linewidth} >{\raggedright\arraybackslash}p{0.16\linewidth} X}
    \toprule
    Setting & AP50 & Prec.50 & Rec.50 \\
    \midrule
    \texttt{T0} (target-only) & $79.1\% \pm 0.014$ & $53.9\% \pm 0.027$ & $91.6\% \pm 0.017$ \\
    \texttt{T1} (\texttt{Y0-3600e200} $\rightarrow$ fine-tune) & $77.3\% \pm 0.041$ & $59.9\% \pm 0.074$ & $86.9\% \pm 0.032$ \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Table~\ref{tab:efficiency} reports inference cost for the detector families and transfer settings at image size $1024$ and batch size $1$. The compact YOLOv8s baseline remains attractive from a deployment perspective: despite higher FLOPs than the MobileNet baselines, it runs faster and uses less peak inference memory in the present environment.

Among the YOLO variants, only the P2-head branch (\texttt{Y1-e200}) clearly increases inference cost, raising parameters, FLOPs, latency, and memory without improving AP50 over \texttt{Y0-3600e200}. The WNIoU-style branch (\texttt{Y2-e200}) keeps essentially the same inference footprint as the baseline, which is consistent with its role as a loss-level training change rather than an architectural modification. The same interpretation applies to \texttt{T0} and \texttt{T1}: source initialization changes the starting weights but not the deployed model structure, so the small latency gap between them should not be read as a structural cost difference.
...
The strongest result in the evaluated design space comes from the compact YOLOv8s baseline. On the curated benchmark, the long-schedule run reaches 84.38\% AP50, placing it above the AP50 values reported in these recent studies on closely related benchmark variants. We do not frame this as a strict SOTA claim because the reconstructed benchmark, checkpoint policy, and evaluation details are not identical across studies.

The MobileNet-based two-stage branch provides a useful reference, especially on the broader full protocol, but it does not match the compact YOLO baseline. The local YOLO modifications do not reverse that result. Under repeated-seed 50-epoch checks, none of the internal YOLO variants produces a robust held-out-test AP50 gain over the baseline, and the matched 200-epoch reruns keep the same overall ranking.

Protocol choice changes the observed ranking enough that it cannot be treated as a reporting detail. The full seven-class protocol stays below the curated benchmark even after the extra 200-epoch rerun, and the broader protocol does not benefit from a longer schedule in the same way as the curated benchmark. The VNWoodKnot study points in the same direction. Source initialization improves optimization and slightly improves the validation average across four seeds, yet the held-out-test aggregate favors \texttt{T0} and shows greater variance for \texttt{T1}.

The ablation results point to the same conclusion. The P2-head branch remains below the compact baseline on AP50 under both short and long schedules, even though it stays competitive on mAP50--95. The WNIoU-style long-schedule rerun comes closer, but it also remains below the baseline.
...
On VNWoodKnot, the representative seed-42 run favored source-initialized fine-tuning, and the four-seed validation average also remained slightly higher for \texttt{T1}. The repeated-seed held-out-test aggregate, however, favored target-only training: \texttt{T0} averaged 79.09\% AP50, whereas \texttt{T1} averaged 77.30\%. The target task is learnable, and transfer remains worth studying, but the present evidence does not support a robust held-out-test AP50 gain for source initialization under the 50-epoch budget used here.
...
\authorcontributions{Khanh Nguyen-Trong conceived the study, designed the methodology, implemented the code, ran the experiments, analyzed the results, and wrote the manuscript.}

\availabilityofdataandmaterials{The source code is available in the supplementary materials at
\href{https://github.com/khanhnt/wood-defect-detection}{Source code}.}

\ethicsapproval{Not applicable.}

\conflictsofinterest{The authors declare that they have no conflicts of interest to report regarding the present study.} 
...
% \label{app:app1dot1}
This appendix reports the repeated-seed matrices underlying the robustness summaries in Section~\ref{sec:multiseed-results}.

\begin{table*}[ht]
  \centering
  \caption{Repeated-seed in-domain YOLO summary on the curated benchmark under the 50-epoch budget. Native validation AP50 comes from the best-checkpoint Ultralytics training logs, whereas held-out test metrics come from the shared repo evaluator. All values are mean $\pm$ standard deviation over seeds 42, 43, 44, and 45.}
...
\begin{table*}[ht]
  \centering
  \caption{Per-seed held-out-test AP50 for the 50-epoch in-domain YOLO checks on the curated benchmark. All values are reported on the shared held-out test split using the shared repo evaluator. Reported values are mean $\pm$ standard deviation over seeds 42, 43, 44, and 45.}
...
\begin{table*}[ht]
  \centering
  \caption{Per-seed held-out-test AP50 for the 50-epoch target-domain adaptation runs on VNWoodKnot. All values are reported on the shared held-out target-domain test split using the shared repo evaluator. Reported values are mean $\pm$ standard deviation over seeds 42, 43, 44, and 45.}
...
% \subsection[\appendixname~\thesection]{Supplementary Robustness Tables} \label{sec:appendix-robustness}
%\appendix
%\section{Supplementary Robustness Tables}
%\label{sec:appendix-robustness}
...
% \begin{table*}[ht]
%   \centering
%   \caption{Per-seed AP50 for the 50-epoch in-domain YOLO checks on the curated benchmark.}
%   \label{tab:app-curated-perseed}
%   \scriptsize
%   \setlength{\tabcolsep}{4pt}
%   \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.26\textwidth} c c c c X}
...
% \begin{table*}[ht]
%   \centering
%   \caption{Per-seed AP50 for the 50-epoch target-domain adaptation runs on VNWoodKnot. Validation AP50 comes from the native YOLO training logs, and held-out test AP50 comes from the shared repo evaluator.}
%   \label{tab:app-transfer-perseed}
%   \scriptsize
%   \setlength{\tabcolsep}{4pt}
%   \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.28\textwidth} c c c c X}
